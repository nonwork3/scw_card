...
function addField(type) {
  ...
  row.innerHTML = `<input type="${c.type}" class="${c.cls}" placeholder="${c.ph}" aria-labelledby="${type}-group-label" autocomplete="off"><button type="button" class="btn-remove" title="ลบ" onclick="removeFieldRow(this)">✕</button>`;
...
function downloadSignature() {
  const v = getValues();
  downloadHtmlFile(generateSignature(v), 'signature-' + (v.slug || 'scw') + '.html');
}
